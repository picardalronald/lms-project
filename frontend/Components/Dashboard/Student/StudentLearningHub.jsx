  ...
  return (
    <main className="p-10 pt-2 max-w-[1600px] mx-auto">
      ...
      <section className="mb-8">
        <h1 className="text-4xl font-serif font-semibold mb-2 text-[#2D362F]">Learning Hub</h1>
      ...
      <section className="mb-12 bg-[#FBFBFA] border border-gray-100 rounded-3xl p-8">
        <div className="flex items-center gap-2 mb-6">
          <Calendar size={18} className="text-[#3D5A4C]" />
          <h3 className="text-sm font-bold text-[#2D362F] uppercase tracking-widest">Due this week</h3>
        </div>
      ...
      {/* 3. My courses Grid */}
      <section>
        <h3 className="text-xl font-bold text-[#2D362F] mb-8">My courses</h3>
...
const DueTaskCard = ({ tag, title, due }) => (
  <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm transition-transform hover:translate-y-[-4px]">
    <div className="flex items-center gap-2 mb-3">
      <div className="w-2 h-2 rounded-full bg-orange-400"></div>
      <span className="text-[10px] font-bold text-teal-700 uppercase">{tag}</span>
    </div>
    <h4 className="text-sm font-bold text-[#2D362F] mb-2 leading-tight">{title}</h4>
...
const CourseHubCard = ({ code, title, prof, progress, percentage, nextUp, nextDue }) => (
  <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-full">
    ...
    <div className="bg-[#3D967C] p-8 text-white">
    ...
    <div className="p-8 flex-1">
      <div className="flex justify-between items-end mb-4">
        <p className="text-sm font-medium text-gray-500">{prof}</p>
      ...
      <div className="w-full bg-[#F1F3F0] h-[6px] rounded-full overflow-hidden mb-8">
        <div 
      ...
      {/* Footer / Next Up Section */}
      <div className="bg-[#FBFBFA] -mx-8 -mb-8 p-6 px-8 border-t border-gray-50 flex justify-between items-center">
        <div>
          <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Next up</p>
          <p className="text-sm font-bold text-[#2D362F]">{nextUp}</p>
          <p className="text-[10px] text-gray-400">{nextDue}</p>
        </div>
        <button className="bg-[#3D5A4C] hover:bg-[#2D362F] text-white px-5 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all">
