    ...
    <main className="min-h-screen bg-[#F8F9F5]">
      ...
      <section className="px-6 pt-24 pb-32 text-center max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-4 mb-24">
          <a href='/Login' className="rounded-2xl bg-[#3D967C] px-10 py-4 font-bold text-white transition-all hover:bg-[#2D362F] flex items-center gap-2">
            Login to portal <ArrowRight size={18} />
          </a>
          <button className="rounded-2xl border border-gray-200 bg-white px-10 py-4 font-bold text-gray-600 transition-all hover:bg-gray-50">
            Explore features
          </button>
        </div>
      ...
      {/* 3. Features Section */}
      <section className="px-6 py-24 bg-white/50 border-t border-gray-100">
        <div className="max-w-6xl mx-auto">
            <div className="text-center mb-16">
                <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-[#3D967C]">Everything in one portal</span>
                <h2 className="text-4xl font-serif font-bold text-[#2D362F] mt-4">Built around your week.</h2>
            </div>
      ...
      {/* 4. Bottom CTA Section (image_d82d33.png style) */}
      <section className="px-6 py-32">
        <div className="max-w-6xl mx-auto bg-white rounded-[48px] p-16 border border-gray-100 shadow-sm flex flex-col lg:flex-row items-center gap-16">
            <div className="flex-1">
                ...
                <button className="bg-[#3D967C] text-white px-8 py-4 rounded-2xl font-bold hover:bg-[#2D362F] transition-all">
...
const StatCard = ({ val, label }) => (
  <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all cursor-default">
    ...
    <div className="text-[10px] uppercase tracking-widest text-gray-400 font-bold mt-2">{label}</div>
  </div>
);
...
const FeatureCard = ({ icon, title, desc }) => (
  <div className="group bg-white p-10 rounded-[40px] border border-transparent hover:border-gray-100 hover:shadow-[0_20px_50px_rgba(0,0,0,0.04)] transition-all duration-500">
    ...
    <h3 className="text-xl font-bold text-[#2D362F] mb-4 font-serif">{title}</h3>
